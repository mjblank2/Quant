...
class DeflatedSharpeRatio:
    ...
    def compute_deflated_sharpe(
        ...
        psr = stats.norm.cdf(
            (sharpe - self.benchmark_sharpe) / np.sqrt(variance_estimate)
        )

        # Deflated Sharpe Ratio adjusts for multiple testing
        if n_trials > 1:
            # Expected maximum Sharpe under null hypothesis
            expected_max_sharpe = self.benchmark_sharpe + np.sqrt(
                variance_estimate
            ) * self._expected_maximum_of_normals(n_trials)

            # DSR is probability that observed Sharpe > expected maximum under null
            dsr = stats.norm.cdf(
                (sharpe - expected_max_sharpe) / np.sqrt(variance_estimate)
            )
        else:
            dsr = psr
    ...
    def _expected_maximum_of_normals(self, n: int) -> float:
        ...
        if n <= 10:
            ...
            return np.sqrt(2 * np.log(n)) - (np.log(np.log(n)) + np.log(4 * np.pi)) / (
        ...
        return np.sqrt(2 * np.log(n)) - (
            np.log(np.log(n)) + np.log(4 * np.pi) - 2 * gamma
        ) / (2 * np.sqrt(2 * np.log(n)))
...
class CombinatorlialPurgedCV:
    ...
    def split(
        ...
        for i in range(self.n_splits):
            ...
            test_start_idx = (
                i * (n_dates - test_days) // (self.n_splits - 1)
                if self.n_splits > 1
                else 0
            )
            ...
            train_dates = unique_dates[:purge_start] + unique_dates[embargo_end:]
    ...
    def validate_splits(
        self, splits: List[Tuple[np.ndarray, np.ndarray]], timestamps: pd.Series
    ) -> Dict[str, Any]:
        """Validate that splits properly implement purging and embargo"""
        validation_results = {"valid": True, "warnings": [], "stats": {}}
        ...
        for i, (train_idx, test_idx) in enumerate(splits):
            ...
            if set(train_idx).intersection(set(test_idx)):
                ...
                validation_results["warnings"].append(
                    f"Split {i}: Train/test overlap detected"
                )
            ...
            if len(train_idx) > 0 and len(test_idx) > 0:
                ...
                if max_train_date > min_test_date:
                    validation_results["warnings"].append(
                        f"Split {i}: Train data contains future of test data"
                    )
...
class WalkForwardAnalysis:
    ...
    def generate_windows(
        ...
        while (
            current_date
            + pd.DateOffset(months=self.train_window_months + self.test_window_months)
            <= end_date
        ):
    ...
    def run_analysis(
        self, data: pd.DataFrame, model_func, target_col: str = "target"
    ) -> Dict[str, Any]:
        """
        ...
        for i, window in enumerate(windows):
            # Split data
            train_mask = (data.index >= window["train_start"]) & (
                data.index < window["train_end"]
            )
            test_mask = (data.index >= window["test_start"]) & (
