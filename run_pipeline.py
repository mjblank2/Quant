...
def _check_dependencies() -> tuple[bool, str]:
    ...
    try:
        import sqlalchemy  # noqa: F401  # type: ignore
    except ImportError:
...
def _run_alembic_upgrade() -> tuple[bool, str]:
    ...
    try:
        ...
        try:
            from config import DATABASE_URL  # noqa: F401  # type: ignore
...
def _import_modules() -> tuple[bool, dict, str]:
    ...
    try:
        ...
        from config import ENABLE_DATA_VALIDATION  # noqa: F401  # type: ignore
        ...
        try:
            ...
            log.warning(
                "Could not import 'models.ml', likely missing optional ML libraries like xgboost. "
                f"ML steps will be skipped. Error: {e}"
            )
...
def _call_with_optional_date(
    ...
    try:
        ...
        log.debug(
            "Function %s rejected kwargs %s (%s); retrying without extras",
            func,
            kwargs,
            exc,
        )
...
def main(
    ...
    try:
        ...
        try:
            ...
            if target_date and (
                "date" in sig.parameters
                or "as_of" in sig.parameters
                or "target_date" in sig.parameters
            ):
                kwargs: dict[str, object] = {}
                ...
                log_market_calendar_status(**kwargs)  # type: ignore[arg-type]
            else:
        ...
        if not ignore_market_hours:
            if target_date and target_date < date.today():
                should_run = True
                calendar_reason = (
                    f"Targeting prior day {target_date.isoformat()} — bypassing 'market not closed yet' gate"
                )
            else:
                ...
                try:
                    ...
                    if target_date and (
                        ...
                        if "date" in sig.parameters:
                            ...
                            kwargs["target_date"] = target_date
                        should_run, calendar_reason = should_run_pipeline(**kwargs)  # type: ignore[arg-type]
                    else:
    ...
    try:
        if "sqlite" in DATABASE_URL.lower():
            ...
            lock_file = os.path.join(_tempfile.gettempdir(), "pipeline_lock")
    ...
    try:
        ...
        try:
            ...
            if modules.get("generate_today_trades"):
                ...
                if (
                    sync_broker
                    and modules.get("sync_trades_to_broker")
                    and trades is not None
                    and getattr(trades, "empty", False) is False
                ):
                    log.info(" Stage 5: Syncing trades to broker")
                    ids = (
                        trades["id"].dropna().astype(int).tolist()
                        if "id" in getattr(trades, "columns", [])
                        else []
                    )
                    if ids:
        ...
        # Outer exception handler around pipeline initialization
        log.exception(" Pipeline failed during setup.")
        return False


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run the EOD pipeline with optional date overrides."
    )
    parser.add_argument(
        "--sync-to-broker",
        action="store_true",
        help="Also sync generated trades to broker.",
    )
    parser.add_argument(
        "--ignore-market-hours",
        "--force",
        dest="ignore_market_hours",
        action="store_true",
    ...
    parser.add_argument(
        "--yesterday",
        action="store_true",
        help="Run as if for the previous market day.",
    )
    parser.add_argument(
        "--target-date",
        type=_parse_iso_date,
        help="Run as if for the specified date (YYYY-MM-DD).",
    )
