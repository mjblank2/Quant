...
def test_tca_models():
    ...
    try:
        from tca import SquareRootLaw, ExecutionAnalyzer, TransactionCostModel
...
def test_validation_tools():
    ...
    try:
        ...
        print(f"✓ Deflated Sharpe Ratio: {dsr_result['dsr']:.3f} (Sharpe: {dsr_result['sharpe']:.3f})")
...
def test_alternative_data():
    ...
    try:
        ...
        if sentiment_result:
            ...
            print(f"✓ Sentiment analysis: {first_result.score:.3f} (confidence: {first_result.confidence:.3f})")
    ...
    try:
        ...
        try:
            ...
            if "DATABASE_URL" in str(e):
                print("✓ Alpha Factory imports successful (DB connection expected to fail in test)")
            else:
                raise e
