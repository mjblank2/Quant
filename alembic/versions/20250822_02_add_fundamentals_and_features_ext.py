"""Fundamentals and alternative signals tables with feature extensions.
...
def upgrade() -> None:
    """Create fundamentals and alt_signals tables and extend features."""
    op.create_table(
        "fundamentals",
    ...
    with op.batch_alter_table("features") as batch_op:
        ...
        batch_op.add_column(
            sa.Column("f_debt_to_equity", sa.Float(), nullable=True)
        )
        batch_op.add_column(sa.Column("f_roa", sa.Float(), nullable=True))
        batch_op.add_column(sa.Column("f_gm", sa.Float(), nullable=True))
        batch_op.add_column(
            sa.Column("f_profit_margin", sa.Float(), nullable=True)
        )
        batch_op.add_column(
            sa.Column("f_current_ratio", sa.Float(), nullable=True)
        )


def downgrade() -> None:
    """Revert feature extensions and drop new tables."""
    with op.batch_alter_table("features") as batch_op:
