...
DATABASE_URL = os.environ.get('DATABASE_URL') # Injected by Render
...
def ingest_daily_data():
    ...
    try:
        ...
        print(f"Retrieved {len(bars_df)} total bars from Alpaca.")
        ...
        bars_df.rename(columns={'timestamp': 'timestamp', 'trade_count': 'trade_count', 'vwap': 'vwap'}, inplace=True)
        ...
        result_message = f"Backfill complete. Saved {len(bars_df)} bars to the database."
