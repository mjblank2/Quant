...
def build_features(batch_size: int = 200, warmup_days: int = 90) -> pd.DataFrame:
    ...
    for i in range(0, len(syms), batch_size):
        ...
        for sym, g in px.groupby('symbol'):
            ...
            f_sym = fnd[fnd['symbol'] == sym][['available_at','pe_ttm','pb','ps_ttm','debt_to_equity','return_on_assets','gross_margins','profit_margins','current_ratio']].sort_values('as_of')
            if not f_sym.empty:
                g = pd.merge_asof(
                    g.sort_values('ts'),
                    f_sym.rename(columns={'available_at':'avail_ts'}).sort_values('ts_fnd'),
                    left_on='ts',
                    right_on='avail_ts',
                    direction='backward'
                )
            ...
            essential = ['ret_1d','ret_5d','ret_21d','mom_21','mom_63','vol_21','rsi_14','turnover_21','size_ln']
            fcols = essential + ['symbol','ts','f_pe_ttm','f_pb','f_ps_ttm','f_debt_to_equity','f_roa','f_gm','f_profit_margin','f_current_ratio']
            # Keep rows where essentials exist; allow fundamentals to be NaN
            keep = g[ ['symbol','ts'] + essential ].dropna().index
            g2 = g.loc[keep, fcols].copy()
