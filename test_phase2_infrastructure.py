...
class TestInstitutionalIngest(unittest.TestCase):
    ...
    def test_health_check_structure(self):
        ...
        with patch(
            "data.institutional_ingest.get_timescaledb_info"
        ) as mock_info, patch(
            "data.institutional_ingest.run_validation_pipeline"
        ) as mock_validation, patch(
            "data.institutional_ingest.engine"
        ):

            mock_info.return_value = {"enabled": True, "available": True}
            mock_validation.return_value = MagicMock(
                passed=True, warnings=[], errors=[], metrics={}
            )
            ...
            self.assertIn(
                health["overall_status"], ["HEALTHY", "WARNING", "CRITICAL", "ERROR"]
            )
