...
def _submit_order_fix(symbol: str, qty: int, side: str, arrival_price: float | None = None, client_order_id: str | None = None) -> str | None:
    ...
    try:
        ...
        if not connector or not connector.session_active:
            ...
            return _submit_order_alpaca_rest(symbol, qty, side, arrival_price, client_order_id)
...
def _submit_order(symbol: str, qty: int, side: str, arrival_price: float | None = None, client_order_id: str | None = None) -> str | None:
...
def sync_trades_to_broker(trade_ids: List[int]) -> Dict[int, str]:
    ...
    with SessionLocal() as s:
        ...
        for t in trades:
            try:
                oid = _submit_order(t.symbol, t.quantity, t.side, t.price, getattr(t, 'client_order_id', None))
                if oid:
                    t.status = "submitted"
                    t.broker_order_id = oid
                    results[t.id] = f"submitted:{oid}"
                    log.info(f"Order submitted: {t.symbol} {t.quantity} {t.side} -> {oid}")
                else:
                    t.status = "error"
                    results[t.id] = "submit_failed"
                    log.error(f"Failed to submit order: {t.symbol} {t.quantity} {t.side}")
            except Exception as e:
                t.status = "error"
...
def schedule_and_execute_child_orders(parent_trades: List[int], execution_style: str = "twap") -> Dict[str, any]:
    """
    ...
    try:
        ...
        try:
            with engine.begin() as con:
                ...
                # Insert child orders
                child_orders_df.to_sql('child_orders', con, if_exists='append', index=False)
        
        except Exception as e:
            log.error(f"Failed to store child orders: {e}")
            return {'status': 'error', 'message': f'Failed to store child orders: {e}'}
        
        log.info(f"Scheduled {len(child_orders_df)} child orders for {execution_style} execution")
