...
class TestCovarianceModels:
    ...
    def test_ewma_covariance(self):
        ...
        np.random.seed(42)
        returns = pd.DataFrame(np.random.randn(100, 5) * 0.02, columns=SAMPLE_SYMBOLS)
...
class TestPortfolioOptimization:
    ...
    def test_mvo_fallback_when_cvxpy_unavailable(self):
        ...
        with patch("portfolio.mvo._latest_prices") as mock_prices, patch(
            "portfolio.mvo._adv20"
        ) as mock_adv, patch("portfolio.mvo.synthesize_covariance") as mock_cov:

            mock_prices.return_value = pd.Series(
                [150, 100, 200, 80, 300], index=SAMPLE_SYMBOLS
            )
...
class TestExecutionAlgorithms:
    ...
    def test_twap_scheduling(self):
        ...
        # Sample trade
        trade = pd.Series(
            {"id": 1, "symbol": "AAPL", "side": "buy", "quantity": 1000, "price": 150.0}
        )
    ...
    def test_vwap_scheduling_fallback(self):
        ...
        with patch.object(
            scheduler, "_get_intraday_volume_profile", return_value=pd.DataFrame()
        ):
            trade = pd.Series(
                {
                    "id": 1,
    ...
    def test_implementation_shortfall_scheduling(self):
        ...
        with patch.object(
            scheduler,
            "_get_market_data",
            return_value={"adv": 1000000, "volatility": 0.20},
        ):
            trade = pd.Series(
                {
                    "id": 1,
...
if __name__ == "__main__":
    ...
    try:
        ...
        test_returns = pd.DataFrame(
            np.random.randn(50, 3) * 0.02, columns=["A", "B", "C"]
        )
